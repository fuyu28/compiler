...
void compound_st(int blab, int clab)
 {
  ...
  size = local_size;
  local_decls();
  while(tok != SRBRACE) st(blab,clab);
  tok = scan();
  end_block();
  local_size = size;
 }
...
void do_st() {
  ...
  clab = get_inlabel();
  st(blab,clab);
  if(tok != SWHILE) error(" 'while' Expected");
  tok = scan();
  if(tok != SLPAREN) error(" '(' Expected");
  tok = scan();
  gen_code0i("L%d:",clab);
...
void for_st() {
  ...
  if(tok != SLPAREN) error(" '(' Expected");
  tok = scan();
  if(tok != SSEMI) expcode();
  if(tok != SSEMI) error(" ';' Expexted");
  tok = scan();
  gen_code0i("L%d:",tlab);
  if(tok != SSEMI) {
    expcode();
...
void switch_st(int clab)
 {
  ...
  while(tok != SRBRACE) {
    if(tok == SDEFAULT) {
      ...
      tok = scan();
      if(dlab != 0) error("Find Second Default");
      dlab = get_inlabel();
      gen_code0i("L%d:",dlab);
     }
     else if(tok == SCASE) {
      ...
      do {
        tok = scan();
        if(tok != SCONST) error("Constant Expected");
...
void goto_st() {
  if(tok != SIDENT) error("Label Expected");
...
void st(int blab, int clab)
 {
  switch(tok) {
    case SLBRACE   :  tok = scan();
                      compound_st(blab,clab);
                      break;
    case SIF       :  tok = scan();
                      if_st(blab,clab);
                      break;
    case SWHILE    :  tok = scan();
                      while_st();
                      break;
    case SDO       :  tok = scan();
                      do_st();
    ...
    case SFOR      :  tok = scan();
                      for_st();
                      break;
    case SSWITCH   :  tok = scan();
                      switch_st(clab);
                      break;
    case SBREAK    :  tok = scan();
                      break_cont_st(blab);
                      break;
    case SCONTINUE :  tok = scan();
                      break_cont_st(clab);
                      break;
    case SRETURN   :  tok = scan();
                      return_st();
                      break;
    case SSEMI     :  tok = scan();
                      break;
