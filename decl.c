...
void push_scope() {
  scope_root = get_snode(scope_root);
 }


void pop_scope() {
  scope_root = scope_root->snextp;
 }
...
struct ID **sub_search_id(char *id, struct ID **root)
 {
  ...
  if(*root == NULL || (i = strcmp(id,(*root)->sid)) == 0)
    return(root);
   else if(i<0) return(sub_search_id(id,&((*root)->lidp)));
...
void sub_insert_id(struct ID **root, char *id, struct TYPE *tp, int at,int ad)
 {
  ...
  if(*idpp == NULL) *idpp = get_inode(id,tp,at,ad);
   else if((*idpp)->attr == EXTERNAL && at == GLOBAL) {
    (*idpp)->attr = GLOBAL;
    if((*idpp)->itp->ttype != tp->ttype ||
       (*idpp)->itp->tsize != tp->tsize)
      error(" Double typed Id is found");
    (*idpp)->itp = tp;
   }
   else error("Double defined Id.");
 }
...
void size_check(struct TYPE *tp)
 {
  switch(tp->ttype) {
    case TARRAY :
      size_check(tp->ttp);
      if(tp->ttp->tsize == 0) error("Data size is Undefined");
      tp->tsize = tp->asize * tp->ttp->tsize;
     break;
    case TFUNCT :
      size_check(tp->ttp);
      if(tp->ttp->ttype == TFUNCT || tp->ttp->ttype == TARRAY
        || tp->ttp->ttype == TSTRUCT) error("Illegal return type");
     break;
    case TPOINTER :
      size_check(tp->ttp);
     break;
...
struct TYPE *tpspec() {
  ...
  switch(tok) {
    case SCHAR : tok = scan(); return(charp);
    case SINT  : tok = scan(); return(intp);
    case SSTRUCT:
      tok = scan();
      if(tok == SIDENT) {
        tag_name = scan_str;
        ...
        tag_name = NULL;
        if(tok != SLBRACE) error("Struct body Expected");
        tsp = get_tnode(TSTRUCT,0,0,NULL);
       }
      tok = scan();
      while((tps = tpspec()) != NULL) {
        while(tok != SSEMI) {
          tp = subdecls(tps,&id);
          size_check(tp);
          if(id == NULL) error(" Identifier Expected");
          if(tp->ttype == TFUNCT) error("Function is not member");
          if(tp->tsize == 0) error("Member size is Undefined");
          sub_insert_id((struct ID **)&(tsp->ttp),id,tp,
                                              MEMBER,tsp->tsize);
          tsp->tsize += (((tp->tsize + 3) >> 2) << 2);
          if(tok == SCOMMA) {
            tok = scan();
            if(tok == SSEMI) error("Illegal comma");
           }
         }
        tok = scan();
       }
      if(tok != SRBRACE) error(" '}' Expected");
      tok = scan();
      return(tsp);
   }
  return(NULL);
...
struct TYPE *subdecls(struct TYPE *tp, char **id)
 {
  ...
  if(ctp != NULL) for(p = ctp; p->ttp != NULL; p = p->ttp);
  ...
  if(tok == SLPAREN) {
    ...
    if(ctp == NULL) {
      ...
      while(tok == SIDENT) {
        ...
        if(tok == SCOMMA) {
          tok = scan();
          if(tok != SIDENT) error("Illegal comma");
         }
       }
     }
    if(tok != SRPAREN) error(" ')' Expected");
    tok = scan();
    if(ctp == NULL) p = ctp = get_tnode(TFUNCT,0,0,NULL);
     else {
    ...
    if(tok == SCONST) {
     ...
     else if(ctp == NULL) p = ctp = get_tnode(TARRAY,0,0,NULL);
     else error("Array index Expected");
...
void para_decls() {
  ...
  while((tps = tpspec()) != NULL) {
    while(tok != SSEMI) {
      ...
      if(i == para_off) error("It is not Decl. of parameter");
      if(tok == SCOMMA) {
        tok = scan();
        if(tok == SSEMI) error("Illegal comma");
       }
  ...
  for(off = 8, i = 0; i < para_off; i++) {
    if(para_tab[i].ptp == NULL) para_tab[i].ptp = intp;
...
void local_decls() {
  ...
  while((tps = tpspec()) != NULL) {
    while(tok != SSEMI) {
      tp = subdecls(tps,&id);
      size_check(tp);
      if(tp->ttype == TFUNCT) error("Function is local");
      if(tp->tsize == 0) error("Local var. size is Undefined");
      if(id == NULL) error(" Identifier Expected");
...
void ext_decls() {
  ...
  while(tok != SEOF) {
    ...
    if((tps = tpspec()) == NULL) tps = intp;
    while(tok != SEOF && tok != SSEMI) {
      tp = subdecls(tps,&id);
      size_check(tp);
      if((tp->ttype == TARRAY && at == GLOBAL ||
         tp->ttype == TSTRUCT) && tp->tsize == 0)
      ...
      if(at == GLOBAL) {
        if(tp->ttype != TFUNCT) {
          ...
          if(tok == SLBRACE) {
           ...
           else error("Function body Expected");
          pop_scope();
          release();
          goto L;
         }
