...
struct ENODE *get_enode(int o, struct TYPE *tp,int lvf, struct ENODE *lp, struct ENODE *rp)
 {
  struct ENODE *p;
...
struct ENODE *get_enodei(int o, struct TYPE *tp,int lvf, int i, struct ENODE *rp)
 {
  struct ENODE *p;
...
struct ENODE *mkpexp() {
  ...
  switch(tok) {
    case SIDENT :
      ...
      if(idp == NULL)
        if(nscan() == SLPAREN)
          idp = get_inode(scan_str,get_tnode(TFUNCT,0,0,intp),
                          GLOBAL,0);
         else error("Undefined Identifier");
      if(idp->attr == TAG) error("TAG in expression");
      p = get_enode(OIDENT,idp->itp,LV,(struct ENODE *)idp,NULL);
      tok = scan();
     break;
    case SSTR :
      p = get_enode(OSTR,get_tnode(TPOINTER,4,0,charp),
                    RV,(struct ENODE *)scan_str,NULL);
      tok = scan();
     break;
    case SCONST :
    ...
    switch(tok) {
      case SLPAREN :
        ...
        while(tok != SRPAREN) {
          r = mkasexp();
          if(q == NULL) q = r;
           else q = get_enode(OSCOMMA,NULL,RV,q,r);
          if(tok == SCOMMA) {
            tok = scan();
            if(tok == SRPAREN) error("Illegal Comma");
           }
         }
        tok = scan();
      ...
      case SDOT :
        ...
        tok = scan();
        if(tok != SIDENT) error("Identifier Expected");
        idp = *sub_search_id(scan_str,(struct ID **)&p->etp->ttp);
        if(idp == NULL) error("Undefined member");
        p = get_enode(OSACCESS,idp->itp,LV,p,(struct ENODE *)idp);
        tok = scan();
       break;
      case SPOINTTO :
        if(p->etp->ttype != TPOINTER ||
          p->etp->ttp->ttype != TSTRUCT)
          error("Illegal type at struct access");
        tok = scan();
        if(tok != SIDENT) error("Identifier Expected");
        idp = *sub_search_id(scan_str,
                             (struct ID **)&p->etp->ttp->ttp);
        if(idp == NULL) error("Undefined member");
...
struct ENODE *mkuexp() {
  ...
  switch(tok) {
    ...
    case SBAND :
      tok = scan();
      p = mkuexp();
      p = get_enode(OADDRESS,get_tnode(TPOINTER,4,0,p->etp),
                    RV,p,NULL);
     break;
    case SMINUS : case SLNOT : case SBNOT : case SINC : case SDEC :
      if(tok == SMINUS) to = OUMINUS;
       else if(tok == SLNOT) to = OLNOT;
       else if(tok == SBNOT) to = OBNOT;
       else if(tok == SINC)  to = OLINC;
       else to = OLDEC;
      tok = scan();
      p = mkuexp();
      t = p->etp->ttype;
    ...
    default :
      if(istype()) {
        ...
        if(tok == SINC || tok == SDEC) {
          if(tok == SINC) to = ORINC;  else to = ORDEC;
          tok = scan();
          t = p->etp->ttype;
          if(t == TFUNCT || t == TARRAY || t == TSTRUCT)
...
struct ENODE *mkmulexp() {
  ...
  p = mkuexp();
  if(tok != SSTAR && tok != SDIV && tok != SMOD) return(p);
  if(p->etp->ttype != TCHAR && p->etp->ttype != TINT)
    error("Illegal type at multiply operator");
  for(;;) {
    if(tok == SSTAR && nscan() != SASSIGN) op = OMUL;
     else if(tok == SDIV && nscan() != SASSIGN) op = ODIV;
     else if(tok == SMOD && nscan() != SASSIGN) op = OMOD;
     else break;
    tok = scan();
    q = mkuexp();
    if(q->etp->ttype != TCHAR && q->etp->ttype != TINT)
...
struct ENODE *mkaddexp() {
  ...
  p = mkmulexp();
  if(tok != SPLUS && tok != SMINUS) return(p);
  if(p->etp->ttype == TFUNCT ||  p->etp->ttype == TSTRUCT)
     error("Illegal type at aditive operator");
  for(;;) {
    if(tok == SPLUS && nscan() != SASSIGN) op = OADD;
     else if(tok == SMINUS && nscan() != SASSIGN) op = OSUB;
     else break;
    tok = scan();
    q = mkmulexp();
    tq = q->etp->ttype;
...
struct ENODE *mksexp() {
  ...
  p = mkaddexp();
  if(tok != SLSHIFT && tok != SRSHIFT) return(p);
  if(p->etp->ttype != TCHAR && p->etp->ttype != TINT)
    error("Illegal type at shift operator");
  for(;;) {
    if(tok == SLSHIFT && nscan() != SASSIGN) op = OLSHIFT;
     else if(tok == SRSHIFT && nscan() != SASSIGN) op = ORSHIFT;
     else break;
    tok = scan();
    q = mkaddexp();
    if(q->etp->ttype != TCHAR && q->etp->ttype != TINT)
...
struct ENODE *mkueexp() {
  ...
  p = mksexp();
  if(tok != SLESS && tok != SGREAT && tok != SLESSEQ
     && tok != SGREATEQ) return(p);
  if(p->etp->ttype == TFUNCT || p->etp->ttype == TSTRUCT)
    error("Illegal type at unequality operator");
  for(;;) {
    if(tok == SLESS)       op = OLESS;
     else if(tok == SGREAT) op = OGREAT;
     else if(tok == SLESSEQ) op = OLESSEQ;
     else if(tok == SGREATEQ) op = OGREATEQ;
     else break;
    tok = scan();
    q = mksexp();
...
struct ENODE *mkeexp() {
  ...
  p = mkueexp();
  if(tok != SEQUAL && tok != SNOTEQ) return(p);
  if(p->etp->ttype == TFUNCT || p->etp->ttype == TSTRUCT)
    error("Illegal type at equality operator");
  for(;;) {
    if(tok == SEQUAL) op = OEQUAL;
     else if(tok == SNOTEQ) op = ONOTEQ;
     else break;
    tok = scan();
    q = mkueexp();
    if(q->etp->ttype == TFUNCT || q->etp->ttype == TSTRUCT)
...
struct ENODE *mkbaexp() {
  ...
  p = mkeexp();
  if(tok != SBAND) return(p);
  if((t = p->etp->ttype) == TFUNCT || t == TARRAY || t == TSTRUCT)
    error("Illegal type at '&'");
  while(tok == SBAND && nscan() != SASSIGN) {
    tok = scan();
    q = mkeexp();
    if((t = q->etp->ttype) == TFUNCT || t == TARRAY
      || t == TSTRUCT) error("Illegal type at '&'");
...
struct ENODE *mkcexp() {
  ...
  p = mkloexp();
  if(tok != SQUEST) return(p);
  if((t = p->etp->ttype) == TFUNCT || t == TARRAY || t == TSTRUCT)
    error("Illegal type at '?'");
  if(tok == SQUEST) {
    tok = scan();
    q = mkcexp();
    if(tok != SCOLON) error(" ':' Expected");
    tok = scan();
    r = mkcexp();
    if(q->etp->ttype != r->etp->ttype ||
      q->etp->tsize != r->etp->tsize)
...
struct ENODE *mkasexp() {
  ...
  p = mkcexp();
  switch(tok) {
    case SPLUS :    case SMINUS :    case SSTAR :    case SDIV :
    case SMOD :     case SLSHIFT :   case SRSHIFT :  case SBAND :
    case SBEOR :    case SBOR :
      if(nscan() != SASSIGN) break;
      op = tok;
      tok = scan();
    case SASSIGN :
      tok = scan();
      if((t = p->etp->ttype) == TFUNCT || t == TARRAY ||
          t == TSTRUCT) error("Illegal type at Assign");
      q = mkasexp();
      if((t = q->etp->ttype) == TFUNCT || t == TSTRUCT ||
         op != SASSIGN && (t == TARRAY || t == TPOINTER))
        error("Illegal type at Assign");
      switch(op) {
        case SPLUS :    op = OADDAS;     break;
        case SMINUS :   op = OSUBAS;     break;
        case SSTAR :    op = OMULAS;     break;
        case SDIV :     op = ODIVAS;     break;
        case SMOD :     op = OMODAS;     break;
        case SLSHIFT :  op = OLSHIFTAS;  break;
        case SRSHIFT :  op = ORSHIFTAS;  break;
        case SBAND :    op = OBANDAS;    break;
        case SBEOR :    op = OBEORAS;    break;
        case SBOR :     op = OBORAS;     break;
        case SASSIGN :  op = OASSIGN;    break;
       }
