...
void bsscan() {
  ...
  if(ctable[cbuf] == SDIGIT) {
    ...
    if(ctable[cbuf] == SDIGIT) {
      i = (i << 3) + (cbuf - '0');
      cscan();
      if(ctable[cbuf] == SDIGIT) i = (i << 3) + (cbuf - '0');
       else uncscan();
     }
     else uncscan();
    cbuf = i;
...
START:
  while((next_token = ctable[cbuf]) == SNULL) cscan();
  switch(next_token) {
    case SALPHA :
      p = in_str;
      do {
    ...
    case SSQUOTE :
      ...
      next_token = SCONST;
      cscan();
      if(ctable[cbuf] != SSQUOTE) error(" ''' Expected");
      cscan();
     break;
    case SDQUOTE :
      cscan();
      ...
      while(ctable[cbuf] != SDQUOTE) {
        if(cbuf == '\\') bsscan();
        if(ctable[cbuf] == SEOF) error("Illegal string");
        *p++ = cbuf;
        cscan();
       }
      *p = '\0';
      ...
      if(ctable[cbuf] == SSTAR) {
        ...
        do {
          while(ctable[cbuf] != SSTAR && ctable[cbuf] != SEOF)
            cscan();
          if(ctable[cbuf] == SEOF) error("Illegal comment");
          cscan();
